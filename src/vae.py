...
from clustering import kmeans_labels, agglomerative_labels, dbscan_auto
...
class TfidfEmbedder:
    def __init__(self, max_features=8000, min_df=2):
        # improvements that usually help lyrics:
        # - bigrams capture phrases ("broken heart", "love you")
        # - sublinear_tf reduces dominance of repeated words
        # - max_df removes extremely common words that aren't useful
...
class TextVAE(nn.Module):
    def __init__(self, in_dim: int, latent_dim=64, hidden=512, dropout=0.2):
        super().__init__()
...
# training
# -------------------------
def train_text_vae(X: np.ndarray, epochs=40, batch=64, lr=1e-3, beta_max=0.05, warmup=10, device="cpu"):
    """
    Fixes collapse: KL annealing + free-bits, then return L2-normalized mu for clustering.
    """
    model = TextVAE(X.shape[1], latent_dim=64, hidden=512, dropout=0.2).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    x = torch.tensor(X, dtype=torch.float32).to(device)

    free_bits = 0.2  # stops KL from forcing collapse too early

    for ep in range(epochs):
        model.train()
        idx = torch.randperm(x.size(0), device=device)

        beta = beta_max * min(1.0, ep / max(1, warmup))
        ...
        for i in range(0, x.size(0), batch):
            ...
            kl = torch.mean(torch.clamp(kl_per_sample, min=free_bits))
        ...
        if (ep + 1) % 5 == 0:
            with torch.no_grad():
                zstd = mu.std(dim=0).mean().item()
            print(f"[Easy VAE] epoch {ep+1}/{epochs} beta={beta:.3f} latent_std≈{zstd:.4f}")

    model.eval()
    with torch.no_grad():
        h = model.enc(x)
        mu = model.mu(h)
        z = F.normalize(mu, dim=1)   # IMPORTANT for text clustering
...
def run_easy(k=8):
    ...
    m_pca = eval_internal(Xp, y_pca, silhouette_metric="cosine")

    # VAE
    device = "cuda" if torch.cuda.is_available() else "cpu"
    Z = train_text_vae(X, epochs=30, beta_max=0.2, warmup=10, device=device)
    y_vae = kmeans_labels(Z, k)
    m_vae = eval_internal(Z, y_vae, silhouette_metric="cosine")
...
def run_medium(k=6):
    ...
    y_d, eps_used, Zs = dbscan_auto(Z, min_samples=4)
    print("DBSCAN label counts:", dict(zip(*np.unique(y_d, return_counts=True))))
    ...
    rows = [

      {
        ...
        "method": "mmVAE+KMeans",
        **eval_internal(Z, y_k),
        **eval_with_labels(lang_codes, y_k),
      },
      {
        "task": "medium",
        "method": "mmVAE+Agglomerative",
        **eval_internal(Z, y_a),
        **eval_with_labels(lang_codes, y_a),
      },
      {
        "task": "medium",
        "method": f"mmVAE+DBSCAN(eps={eps_used:.3f})",
        **eval_internal(Zs, y_d),          # IMPORTANT: use scaled Z for DBSCAN metrics
        **eval_with_labels(lang_codes, y_d),
      },
    ]
    # for name, y in [("KMeans", y_k), ("Agglomerative", y_a), ("DBSCAN", y_d)]:
    #     rows.append({
    #         "task":"medium",
    #         "method":f"mmVAE+{name}(eps={eps_used:.3f})",
    #         **eval_internal(Zs, y_d),
    #         **eval_with_labels(lang_codes, y_d)
    #       })
